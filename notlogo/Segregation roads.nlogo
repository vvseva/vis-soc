...
to setup
  ...
  ask n-of number patches with [pcolor != brown]
    [ sprout 1 ]
...
to draw-roads
    if mouse-down? [
      ask patch mouse-xcor mouse-ycor [
        set pcolor brown
        display
        ask turtles-here [ find-new-spot ]
      ]
    ]
end
...
to find-new-spot
  let valid-spots patches with [pcolor != brown and not any? turtles-here]
  ifelse any? valid-spots
    [ move-to one-of valid-spots ]
    [ user-message "No empty spots left!" stop ]
end

to update-turtles
  ask turtles [
    ;; 1. Get all potential neighbors within the radius circle
    let candidates other turtles in-radius vision-radius

    ;; 2. Filter them: Keep only those where the line of sight is NOT blocked
    ;;    Note: "myself" refers to the turtle doing the looking.
    let nearby-turtles candidates with [ has-line-of-sight? myself ]

    set similar-nearby count nearby-turtles with [color = [color] of myself]
    set total-nearby count nearby-turtles
    set other-nearby count nearby-turtles with [color != [color] of myself]
...
;; --- IMPROVED REPORTER: Ray Casting ---
to-report has-line-of-sight? [ target ]
  let dist distance target
  let ang towards target

  ;; We check the path in small steps (0.5 patches).
  let step 0.5
  let current-dist step

  while [current-dist < dist] [
    ;; "patch-at-heading-and-distance" handles the dx/dy math internally
    ;; It returns the patch located at the specific angle and distance.
    if [pcolor] of patch-at-heading-and-distance ang current-dist = brown [
      report false ;; Blocked!
    ]
    set current-dist current-dist + step
  ]

  report true ;; path is clear
end
