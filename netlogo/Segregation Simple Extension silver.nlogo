...
globals [
  ...
  temp-venue       ;; tracks the venue currently being placed but not confirmed
]
...
families-own [
  ...
  total-neighbors-vsame
  similar-neighbors-vsame
]
...
to setup
  ...
  ask n-of venues-number patches
  [ sprout-venues 1]
...
to update-families
  ask families [
    ...
    let my-neighbors other families in-radius neighborhood-distance
    ;; count the number of my neighbors that are the same color as me
    set similar-nearby count (my-neighbors)
    ...
    set other-nearby count (my-neighbors)
    with [color != [color] of myself]

    ;; Count VSame the number of individuals from the same group who attended one or more of the same venues as A
    let local-my-venues visited-venues

    ;; distinct-neighbors will contain only the families from 'my-neighbors'
    ;; that share at least one venue with the current family.
    let my-neighbors-vsame my-neighbors with [

        ;; LOGIC EXPLANATION:
        ;; 1. We look at this neighbor's 'visited-venues'
        ;; 2. We check if ANY of those venues exist...
        ;; 3. ...inside 'local-my-venues' (the list belonging to the asking family)

        any? visited-venues with [ member? self local-my-venues ]
    ]

    set total-neighbors-vsame count (my-neighbors-vsame)
    set similar-neighbors-vsame count (my-neighbors-vsame)
    with [color = [color] of myself]
    ...
    set happy? (similar-nearby + similar-neighbors-vsame + 1) / (total-nearby + total-neighbors-vsame + 1) > (my-%-similar-wanted / 100)
  ]
end

to update-globals
  let similar-neighbors sum [similar-nearby] of families
  let total-neighbors sum [total-nearby] of families
...
to place-temporary-venue
  ;; Only execute if the mouse is clicked inside the view
  if mouse-down? [

    ;; 1. Clean up previous visualization
    ask patches with [pcolor = lime] [ set pcolor black ]

    ;; 2. Determine the target patch (this handles the "Centering" logic)
    ...
    ifelse temp-venue = nobody [
      create-venues 1 [
        set temp-venue self
        set shape "house"
        move-to target-patch ;; Snaps to center of the patch
      ]
    ]
    [
      ask temp-venue [
    ...
    ;; 5. Wait for mouse release to prevent "dragging" (ensure discrete clicks)
...
to confirm-venue-placement
  ifelse temp-venue != nobody [
    ...
    ;; 1. "Commit" the venue (it stops being temporary)
    set temp-venue nobody

    ;; 2. Clear visualization (Set background to BLACK)
    ;; We ask all patches to turn black, but we preserve the white spots
    ;; where permanent venues exist (if your model logic requires that).
    ;; Based on your request "ask patches [ set pcolor black]", I will do exactly that:
    ask patches [ set pcolor black ]

    ;; 3. Re-draw existing venues on the map (optional but good for visibility)
    ;; Your original setup code set pcolor white under venues. Let's keep that consistency.
